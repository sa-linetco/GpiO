...
var _unexport = function (_gpioBasePath, number, fn) {
    _write(number, path.join(_gpioBasePath, 'unexport'), function (err) {
...
var _export = function (_gpioBasePath, n, fn) {
    if (exists(path.join(_gpioBasePath, 'gpio' + n))) {
        ...
        _unexport(_gpioBasePath, n, function () {
            _export(_gpioBasePath, n, fn);
        });
    } else {
        logMessage('Exporting gpio' + n);
        _write(n, path.join(_gpioBasePath, 'export'), function (err) {
            // if there's an error when exporting, unexport and repeat
            if (err) _unexport(_gpioBasePath, n, function () {
                _export(_gpioBasePath, n, fn);
            });
...
var GPIO = function (headerNum, opts) {
    ...
    this.gpioBasePath = opts.gpioBasePath || defaultGpioBasePath;
    ...
    this.PATH = {};
    this.PATH.PIN = path.join(this.gpioBasePath, 'gpio' + headerNum);
    this.PATH.VALUE = path.join(this.PATH.PIN, 'value');
    this.PATH.DIRECTION = path.join(this.PATH.PIN, 'direction');
 ...
 */
GPIO.prototype.export = function (fn) {
    _export(this.gpioBasePath, this.headerNum, fn);
};
GPIO.prototype.unexport = function (fn) {
    if (this.valueWatcher) this.valueWatcher.stop();
    _unexport(this.gpioBasePath, this.headerNum, fn);
};
...
GPIO.prototype.reset = function (fn) {
    this.set(0, fn);
};

exports.logging = false;
exports.export = function (headerNum, direction) {
    return new GPIO(headerNum, direction);
};
